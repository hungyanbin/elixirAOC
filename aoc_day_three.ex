defmodule AocDayThree do
  ...
  def execute_command(command, pos \\ [x: 0, y: 0]) do
    <<dir_code::utf8, count::binary>> = command
    draw_wire(direction(dir_code), String.to_integer(count), pos)
  end

  def draw_wire(:left, count, pos) do
  ...
  def draw_wire(:right, count, pos) do
    line = [orientation: "Horizontal", start: pos[:x], end: pos[:x] + count, at: pos[:y]]
    next_pos = [x: pos[:x] + count, y: pos[:y]]
    [line: line, next_pos: next_pos]
  end

  def draw_wire(:up, count, pos) do
    line = [orientation: "Vertical", start: pos[:y], end: pos[:y] - count, at: pos[:x]]
    next_pos = [x: pos[:x], y: pos[:y] - count]
    [line: line, next_pos: next_pos]
  end

  def draw_wire(:down, count, pos) do
    line = [orientation: "Vertical", start: pos[:y], end: pos[:y] + count, at: pos[:x]]
    next_pos = [x: pos[:x], y: pos[:y] + count]
    [line: line, next_pos: next_pos]
  end
  ...
  def execute_command_list([command | other_commands], lines \\ []) do
    line_pos_pair = execute_command(command)
    execute_command_list(other_commands, [line_pos_pair[:line] | lines], line_pos_pair[:next_pos])
  end

  def execute_command_list([command | other_commands], lines, pos) do
    line_pos_pair = execute_command(command, pos)
    execute_command_list(other_commands, [line_pos_pair[:line] | lines], line_pos_pair[:next_pos])
  end

  def execute_command_list([], lines, _) do
    lines
  end
  ...
  defp intersect(v_line, h_line) do
    ...
    if v_line[:at] > h_min && v_line[:at] < h_max &&
      ...
      [x: v_line[:at], y: h_line[:at]]
    end
  end

end

#IO.inspect(AocDayThree.find_closest("R8,U5,L5,D3", "U7,R6,D4,L4"))
#IO.inspect(AocDayThree.find_closest("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83"))
#IO.inspect(AocDayThree.find_closest("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"))
IO.inspect(AocDayThree.find_closest(
